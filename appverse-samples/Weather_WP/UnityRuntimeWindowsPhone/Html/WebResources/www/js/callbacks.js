...
Appverse.UserDidTakeScreenshot = function () {
	Ext.Msg.alert("User Action", "Screenshot taken !");
	console.log("User Action: Screenshot taken !");
};

feedObj = function(cb,cbid,result){
    obj = Ext.create('Showcase.model.Feed');
    obj.set('cbid',cbid);
    obj.set('cb',cb);
    ...
    if(result != null && typeof(result) == "object") {
        ...
        obj.set('result',Showcase.app.getController('Main').syntaxHighlight(result));
    }else{
        obj.set('result',result);
    }
    obj.set('resultShadow',result);
    
    return obj;
}

var submitCallback = function submitCallback(result, id) {
	...
	if(typeof(testCase) != 'undefined' && testCase!=null) {
        testCase.setResult(resultJsonString);
        if(Ext.ComponentQuery.query('test').length)
            Ext.ComponentQuery.query('test')[0].setRecord(testCase);
        
    }
    Showcase.app.getController('Main').showResult();
    Showcase.app.getController('Main').console(feedObj(id,id,result));
    Showcase.app.getController('Main').getLoading().hide();
    if(testCase.get('callback')){
        window[testCase.get('callback')](result,id);
    }
};

var reset = function reset(){
    resultStrMsg="";
    Showcase.app.getController('Main').getMsg().setHtml("");
    Showcase.app.getController('Main').getMsg().hide();
};