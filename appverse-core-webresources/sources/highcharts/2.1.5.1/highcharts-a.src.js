		...
		// setter
			...
			for (key in hash) {
				...
				// apply gradients
					...
					/* don't apply gradients on Android */ // IMPORTANTAREA - Issue #5
					var ua = navigator.userAgent.toLowerCase();
					var isAndroid = ua.indexOf("android") > -1;
					if(value instanceof Object && isAndroid){
						value = 'rgba(84, 125, 173, 0.3)';
					}
...
SVGRenderer.prototype = {
	...
	color: function(color, elem, prop) {
		...
		var ua = navigator.userAgent.toLowerCase();
		var isAndroid = ua.indexOf("android") > -1;
		if (color && color.linearGradient && !isAndroid) {
			var renderer = this,
		...
		// Webkit and Batik can't show rgba.
			...
			/*console.log(colorObject.get('rgb'));*/
			return colorObject.get('rgb');
		} else {/*
			color = 'rgb(255, 255, 155)';
			console.log('last else', color);*/
			return color;
		}
...
function Chart (options, callback) {
	...
	function MouseTracker (chart, options) {
		...
		function setDOMEvents () {
			...
			var mouseMove = function(e) {
				...
				} else { //if (!isOutsidePlot) { ROLA: removed condition so that listeners perform correctly in iOS devices
					// show the tooltip
					onmousemove(e);
				}
	...
	var Legend = function(chart) {
		...
		function renderLegend() {
			...
			/* IMPORTANTAREA - Issue #2 */
			if (legendBorderWidth || legendBackgroundColor) {
				legendWidth += 2 * padding;
				legendHeight += 2 * padding;
				// create the border only if there are any items in the legend
				if(allItems.length){
					if (!box) {
						box = renderer.rect(
							0, 
